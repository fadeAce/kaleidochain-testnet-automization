...
function keyExist(){
    ...
    exist=$(ssh root@${ip} "if [[ -d '/usr/local/kaleido/testnet/node${seq}/testnet/geth/ephemeralkey-${addr}-${height}-${end}' ]]; then
        echo 'exist'
    fi")
}
...
while read IP SEQ ADDR NAME;
    echo "begin with ${IP} ${SEQ} ${ADDR} ${NAME}"
do
