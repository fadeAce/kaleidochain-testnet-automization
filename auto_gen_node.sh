...
do
    ...
    ssh root@${IP} "export NODE_NAME='${NAME}${SEQ}';export CONTAINER='node'${SEQ};wget https://raw.githubusercontent.com/fadeAce/kaleidochain-testnet-automization/master/testnet.sh;chmod +x testnet.sh;bash testnet.sh"
    fi
done < ${filename}
...
function initnode() {
    ...
    seq=$2
    addr=$3
    ssh root@${IP} "mkdir -p /usr/local/kaleido/node${seq}"
    exist=$(ssh root@${IP} "if [ -d "/usr/local/kaleido/testnet/node${seq}/geth/ephemeralkey-${addr}-0-1000000" ];then
        echo 'exist'")
    h=$4
    if [[ "$exist" == "exist" ]]; then
        return 1
    fi
    /tmp/node${seq}.dat.$$ < ssh root@${IP} "docker run --rm -v /usr/local/kaleido/testnet/node${seq}:/root/.ethereum kaleidochain/client:test --testnet makeminerkey --miner ${addr} --coinbase ${addr} --begin ${h}|grep minerkey"
